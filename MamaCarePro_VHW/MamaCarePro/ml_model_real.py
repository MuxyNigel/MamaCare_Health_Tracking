# ml_model_real.py (Fixed version - handles blood pressure format)
import pandas as pd
import numpy as np
...
class RealMaternalRiskModel:
    ...
    def clean_blood_pressure(self, df):
        """Clean blood pressure column - extract systolic from '110/82' format"""
        if 'blood_pressure' in df.columns:
            # Convert blood pressure to numeric by extracting first number before '/'
            df = df.copy()
    ...
    def predict_risk(self, mother_data):
        ...
        for feature in self.feature_names:
            ...
            if feature == 'blood_pressure' and isinstance(value, str) and '/' in value:
